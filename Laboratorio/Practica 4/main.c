...
void setTimeOut(unsigned int timer);
void inc_B16(char * number);
/* =====================================================================*/

unsigned int memory_offset[2] = {0xA7FC, 0x27FC};  /*{0xA800, 0x2800}*/
char memory_offset_str[2][7] = {"0xA7FC\0","0x27FC\0"};

int main(void){
    unsigned int test_addr;
    unsigned char test_bus;
    unsigned char index = 0, state_sRAM, counter;

    puts(">>> Decodificadores\n\rPresione una tecla para continuar...\n\r");
    getchar();
    
    for(counter = 0; counter < 61; counter++) {
        test_bus = TestDataBus(DEFAULT_SEGMENT, memory_offset[index]);
        test_addr = TestAddrBus(DEFAULT_SEGMENT, memory_offset[index]);
        
        puts((char *) memory_offset_str[index]);
        puts(" - ");
        state_sRAM = err_sRAM(test_bus, test_addr, 0);
        
        if (state_sRAM == 2) puts(" ERR        ");
        else if (state_sRAM == 0) puts(" OK         ");
        
        if (index == 1) if (state_sRAM%2 == 0) puts("\n\r");
        
        inc_B16(memory_offset_str[index]);
        memory_offset[index]++;

        index = counter%2;
        setTimeOut(6000);
        
        if(counter%20 == 1 && counter > 1) {
            puts("-- \n");
            getchar();
        }
    }
...
unsigned char err_sRAM(unsigned char statusDataBus, unsigned int statusAddrBus, unsigned char showStatus) {
    ...
    if(statusDataBus >= 0xFF && statusAddrBus >= 0xFF) {
        ...
        return 1;
    } else if(statusAddrBus != 0 || statusDataBus != 0) return 2;
...
/* ================= PRINT ================= */
/**
 * Realiza un conteo para simular una pausa 
*/
void setTimeOut(unsigned int timer) {
    while(--timer > 0);
}
...
/**
 * Incrementa los ultimos 3 digitos de un numero Hexadecimal
 * 0 -> F
 * 
*/
void inc_B16(char * number) {
    ...
    if(number[5]-1 == '9') number[5]+=7;
    else if(number[5]-1 == 'f' || number[5]-1 == 'F') {
        number[4]++;
        number[5] = '0';

        if(number[4]-1 == '9') number[4]+=7;
        else if(number[4]-1 == 'f' || number[4]-1 == 'F') {
            number[3]++;
            number[4] = '0';
        }
    }
